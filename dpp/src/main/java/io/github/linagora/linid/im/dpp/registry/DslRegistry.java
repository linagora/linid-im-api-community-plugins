 ...
 *
 * <p>If a provider configuration changes, the associated datasource and DSL context
 * are recreated. Each provider maintains its own Hikari connection pool.</p>
 */
@Slf4j
@Component
public class DslRegistry {
  ...
  /**
   * Returns a {@link DSLContext} associated with the given provider configuration.
  ...
  public DSLContext getDsl(ProviderConfiguration config) {
    ...
    synchronized (name.intern()) {
      ...
      String url = config.getOption("url")
          .orElseThrow(() -> new IllegalArgumentException("Database URL is mandatory"));
      hikariConfig.setJdbcUrl(url);

      String username = config.getOption("username")
          .orElseThrow(() -> new IllegalArgumentException("Database username is mandatory"));
      hikariConfig.setUsername(username);

      String password = config.getOption("password")
          .orElseThrow(() -> new IllegalArgumentException("Database password is mandatory"));
